 ...
 * - core is not needed. Number, Array etc should only be properties of the 
 *   global object, not heap variables as well. The current implem has a bug in 
 *   the rare case when one of them is redefined:
 *   Number = "foo"; var bug = this.Number;
 *   The type of bug is function and it should be string.
 */
...
//    // var fs = require('fs');
//    // var fd1 = fs.openSync("debug.txt", "w", mode=0777);
//    // print = function(str) {
//    //   fs.writeSync(fd1, str+"\n", null, encoding='utf8');  
//    // }
//  };
...
opArit[NULL] = opArit[THIS] = opArit[TRUE] = opArit[FALSE] = NULLARY;
opArit[IDENTIFIER] = opArit[NUMBER] = opArit[STRING] = NULLARY;
opArit[REGEXP] = NULLARY;

opArit[DELETE] = opArit[VOID] = opArit[TYPEOF] = opArit[NOT] = UNARY;
opArit[BITWISE_NOT] = opArit[UNARY_PLUS] = opArit[UNARY_MINUS] = UNARY;
opArit[NEW] = opArit[GROUP] = opArit[INCREMENT] = opArit[DECREMENT] = UNARY;

opArit[BITWISE_OR] = opArit[BITWISE_XOR] = opArit[BITWISE_AND] = BINARY;
opArit[EQ] = opArit[NE] = opArit[STRICT_EQ] = opArit[STRICT_NE] = BINARY;
opArit[LT] = opArit[LE] = opArit[GE] = opArit[GT] = BINARY;
opArit[INSTANCEOF] = opArit[LSH] = opArit[RSH] = opArit[URSH] = BINARY;
opArit[PLUS] = opArit[MINUS] = opArit[MUL] = opArit[DIV] = BINARY;
opArit[MOD] = opArit[DOT] = opArit[AND] = opArit[OR] = BINARY; 
opArit[ASSIGN] = opArit[INDEX] = opArit[IN] = opArit[DOT] = BINARY;

opArit[HOOK] = TERNARY;
opArit[COMMA] = opArit[ARRAY_INIT] = MULTI;
opArit[OBJECT_INIT] = MULTI_OI;
opArit[CALL] = opArit[NEW_WITH_ARGS] = MULTI_CALL;
opArit[FUNCTION] = FUN;
opArit[YIELD] = NULLARY_OR_UNARY;
...
const DOT_PROTO = IF;
opArit[DOT_PROTO] = UNARY;
const ARGUMENTS = FOR;
opArit[ARGUMENTS] = UNARY;
const PLUS_ASSIGN = WHILE;
opArit[PLUS_ASSIGN] = BINARY;
...
Aobj.prototype.forEachOwnProp = function(f) {
  for (var p in this)
    if (this.hasOwnProperty(p) && /(^-number$)|(^-string$)|-$/.test(p))
      f(p);
};
...
Aobj.prototype.updateProp = function(prop, newv) {
  if (this.hasOwnProperty(prop)) {
    if (!avlt(newv, this[prop])) {
      this[prop] = avjoin(this[prop], newv);
      ++timestamp;
      ++uprop;
    }
    return;
...
function avjoin(v1, v2) {
  ...
  var av = new Aval(v1.base | v2.base);
  if (os1l === 0)
...
function initCoreObjs() {
  ...
  var _Object = (function () {
      ...
      // generic_object_av is used to analyze functions that aren't called
      var gobj = new Aobj({proto : opav}), gobjaddr = ++count;
      generic_object_av = new Aval(NOBASEVALS, gobjaddr);
      heap[gobjaddr] = gobj;
  ...
  (function () {
    ...
    var sepav = new Aval(NOBASEVALS, sepaddr);
    heap[sepaddr] = sep;
    ...
    var seaddr = ++count, seav = new Aval(NOBASEVALS, seaddr);
    var seavaddr = ++count;
    heap[seaddr] = se;
    heap[seavaddr] = seav;
    core["SyntaxError"] = seavaddr;
    go.updateProp("SyntaxError-", seav);
    se.updateProp("prototype-", sepav);
    sep.updateProp("constructor-", seav);
    sep.updateProp("name-", ASTR);
  })();
...
// fun. node, array of Aval, timestamp  -> Aval or false
function searchSummary(n, args, ts) {
  var summary = summaries[n.addr], insouts, pair;
  if (summary.ts < ts) return false;
...
(function() {
  ...
  function _index(n, fr) {
    ...
    if (pt === NUMBER) {
      ...
      if (avprop.hasStr()) {
        avobj.forEachObj(function(o) {
            o.forEachOwnProp(function(p) {
                if (propIsString(p)) av = avjoin(av, o.getOwnProp(p));
              });
          });
      }
    }
...
function evalFun(fn, args, withNew) {
  ...
  var tsAtStart = timestamp;
  var result = searchSummary(fn, args, tsAtStart);
...
function evalToplevel(tl) {
  ...
  for (var addr in summaries) {
    ...
    if (!existsSummary(f)) {
      ...
      any_args.unshift(generic_object_av);
